"""Tests for graph_analytics_ai.ai.schema.arango_products
(PRD v0.6 follow-up / Autograph + GraphRAG product detection)."""

from __future__ import annotations

import pytest

from graph_analytics_ai.ai.schema.arango_products import (
    ArangoProductReport,
    AutographProject,
    _match_suffix,
...
class TestDetectAutograph:
    ...
    def test_corpus_only_failed_run(self):
        ...
        proj = detect_arango_products(snap).autograph_projects[0]
    ...
    def test_strong_marker_gate_rejects_decoy(self):
        ...
        assert detect_arango_products(snap).is_empty
    ...
    def test_graphs_detailed_key(self):
        ...
        assert len(detect_arango_products(snap).autograph_projects) == 1
...
class TestSerialization:
    def test_to_dict_round_trip(self):
        ...
        as_dict = detect_arango_products(snap).to_dict()
...
class TestLiveProbeReplay:
    """Replay of the actual addtech-knowledge-graph snapshot.

    The fixture below was captured from scripts/probe_autograph.py
    against the live database. This test pins behavior against the
    real names + naming asymmetries Autograph emits in production.
    """

    def test_live_addtech_snapshot_classification(self):
        snap = _snap(
            collections=[
                # OpenRTB project (complete run)
                "OpenRTB-API-Specification_Entities",
                "OpenRTB-API-Specification_Documents",
                "OpenRTB-API-Specification_Chunks",
                "OpenRTB-API-Specification_Communities",
                "OpenRTB-API-Specification_domains",
                "OpenRTB-API-Specification_modules",
                "OpenRTB-API-Specification_sources",
                "OpenRTB-API-Specification_rags",
        ...
        report = detect_arango_products(snap)
        names = {p.project_name for p in report.autograph_projects}
        assert names == {"OpenRTB-API-Specification", "AT"}, names

        by_name = {p.project_name: p for p in report.autograph_projects}
        openrtb = by_name["OpenRTB-API-Specification"]
        assert openrtb.completeness == "complete"
        assert openrtb.confidence >= 0.95
        assert len(openrtb.implicit_links) == 1

        at_proj = by_name["AT"]
        assert at_proj.completeness == "corpus_only"
        assert any("INCOMPLETE_AUTOGRAPH_RUN" in w for w in at_proj.warnings)
