...
class TestInsightParsing:
    ...
    def setup_method(self):
        ...
        self.generator = ReportGenerator(
            llm_provider=self.mock_llm, use_llm_interpretation=True
        )
...
class TestInsightTypeInference:
    ...
    def test_infer_anomaly(self):
        ...
        assert (
            self.generator._infer_insight_type("Unusual Spike in Node Activity")
            == InsightType.ANOMALY
        )
        assert (
            self.generator._infer_insight_type("Unexpected Outlier Detected")
            == InsightType.ANOMALY
        )
    ...
    def test_infer_pattern(self):
        ...
        assert (
            self.generator._infer_insight_type("Distribution Pattern Identified")
            == InsightType.PATTERN
        )
        assert (
            self.generator._infer_insight_type("Network Trend Analysis")
            == InsightType.PATTERN
        )
    ...
    def test_infer_opportunity(self):
        ...
        assert (
            self.generator._infer_insight_type("Growth Opportunity in Long Tail")
            == InsightType.KEY_FINDING
        )
        assert (
            self.generator._infer_insight_type("Potential for Optimization")
            == InsightType.KEY_FINDING
        )
    ...
    def test_infer_concern(self):
        ...
        assert (
            self.generator._infer_insight_type("Critical Risk Identified")
            == InsightType.KEY_FINDING
        )
        assert (
            self.generator._infer_insight_type("Network Problem Detected")
            == InsightType.KEY_FINDING
        )
    ...
    def test_infer_key_finding_default(self):
        ...
        assert (
            self.generator._infer_insight_type("Top Nodes Identified")
            == InsightType.KEY_FINDING
        )
...
class TestHeuristicInsights:
    ...
    def setup_method(self):
        ...
        self.mock_llm = Mock()
        self.generator = ReportGenerator(
            llm_provider=self.mock_llm, use_llm_interpretation=False
        )
    ...
    def test_wcc_insights_with_singletons(self):
        ...
        results = [{"_key": f"N{i}", "component": 0} for i in range(50)] + [
        ...
        assert any(
            "isolated" in i.description.lower() or "singleton" in i.description.lower()
            for i in insights
        )
    ...
    def test_betweenness_insights_identifies_bridges(self):
        ...
        assert len(insights) >= 1
        assert any(
            "bridge" in i.title.lower() or "critical" in i.title.lower()
            for i in insights
        )
