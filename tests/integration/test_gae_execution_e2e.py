...
class TestGAEExecutionE2E:
    ...
    def test_complete_gae_analysis_with_metrics(
        self,
        check_env_vars,
        temp_output_dir
    ):
        ...
        assert result.status == AnalysisStatus.COMPLETED, f"Analysis failed: {result.error_message}"
    ...
    def test_multiple_algorithms_with_summary(
        self,
        check_env_vars,
        temp_output_dir
    ):
        ...
        summary = ExecutionSummary(
            ...
            templates_succeeded=len([r for r in results if r.status == AnalysisStatus.COMPLETED]),
            templates_failed=len([r for r in results if r.status != AnalysisStatus.COMPLETED])
        )
