...
class TestWorkflowE2E:
    ...
    def test_complete_workflow_with_real_services(
        ...
        assert result.status == WorkflowStatus.COMPLETED, f"Workflow failed: {result.error_message}"
        ...
        assert Path(result.use_cases_path).exists(), f"Use cases file not found: {result.use_cases_path}"
        
        assert result.schema_path is not None, "Schema analysis was not generated"
        assert Path(result.schema_path).exists(), f"Schema file not found: {result.schema_path}"
        
        assert result.requirements_path is not None, "Requirements were not generated"
        assert Path(result.requirements_path).exists(), f"Requirements file not found: {result.requirements_path}"
        
        # Verify completed steps
        assert len(result.completed_steps) == 7, f"Expected 7 completed steps, got {len(result.completed_steps)}"
        expected_steps = [
            "parse_documents",
        ...
        for step in expected_steps:
            assert step in result.completed_steps, f"Step '{step}' not in completed steps"
        ...
        assert "Product Requirements Document" in prd_content or "Requirements" in prd_content
