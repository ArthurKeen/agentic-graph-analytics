...
class UseCaseGenerator:
    ...
    def _use_case_from_suggestion(
        ...
        if schema_analysis.key_entities:
            data_needs.append(f"Vertex collections: {', '.join(schema_analysis.key_entities)}")
        if schema_analysis.key_relationships:
            data_needs.append(f"Edge collections: {', '.join(schema_analysis.key_relationships)}")
    ...
    def _extract_data_needs(self, text: str, extracted: ExtractedRequirements) -> List[str]:
