...
NOTE: This is the **fallback** implementation. The canonical
detector now lives in ``schema_analyzer.arango_products`` (upstream
``arangodb-schema-analyzer``); the public shim
``graph_analytics_ai.ai.schema.arango_products`` prefers the
upstream symbols and only re-exports from this module when the
upstream version is missing (older analyzer install, or analyzer
not installed at all). The two implementations MUST stay
structurally identical so the bundle shape doesn't change with
the source of truth.
...
def _classify_project(prefix: str, bucket: dict[str, Any]) -> AutographProject:
    ...
    corpus_score = len(vertex_suffixes & _CORPUS_VERTEX_SUFFIXES) / len(
        _CORPUS_VERTEX_SUFFIXES
    )
