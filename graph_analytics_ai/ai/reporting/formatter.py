...
class ExecutionReportFormatter:
    ...
    def format_report(self, summary: ExecutionSummary) -> str:
        ...
        # Timing Breakdown
        if self.config.should_include(ReportSection.TIMING_BREAKDOWN) and summary.timing_breakdown:
            sections.append(self._format_timing_breakdown(summary.timing_breakdown))
        
        # Cost Analysis
        if (self.config.should_include(ReportSection.COST_ANALYSIS) and 
            self.config.include_costs and 
        ...
        # Error Log
        if (self.config.should_include(ReportSection.ERROR_LOG) and 
            summary.errors and 
            self.config.include_error_details):
            sections.append(self._format_error_log(summary))
        
        # Raw Metrics
        if self.config.should_include(ReportSection.RAW_METRICS) and self.config.include_raw_metrics:
    ...
    def _format_title(self, summary: ExecutionSummary) -> str:
        ...
        if self.config.show_timestamps:
            ...
            if summary.started_at:
                lines.append(f"**Started:** {summary.started_at.strftime('%Y-%m-%d %H:%M:%S')}  ")
    ...
    def _format_executive_summary(self, summary: ExecutionSummary) -> str:
        ...
        if summary.cost_breakdown and self.config.include_costs:
            lines.append(f"**Total Cost:** ${summary.cost_breakdown.total_cost_usd:.4f} USD")
    ...
    def _format_cost_breakdown(self, cost: CostBreakdown) -> str:
        ...
        lines.append(f"| Engine Deployment | ${cost.engine_deployment_cost_usd:.{decimals}f} |")
        ...
        # Add cost per minute
        cost_per_minute = cost.runtime_cost_usd / cost.runtime_minutes if cost.runtime_minutes > 0 else 0
        lines.extend([
            "",
    ...
    def _format_performance_metrics(self, summary: ExecutionSummary) -> str:
        ...
        if summary.total_execution_time_seconds > 0:
            vertices_per_sec = summary.total_vertices_processed / summary.total_execution_time_seconds
            edges_per_sec = summary.total_edges_processed / summary.total_execution_time_seconds
