...
class TraceCollector:
    ...
    def finalize(self) -> WorkflowTrace:
        ...
        if self.trace.events:
            start_event = next(
                (e for e in self.trace.events if e.event_type == TraceEventType.WORKFLOW_START),
                None
            )
            end_event = next(
                (e for e in reversed(self.trace.events) if e.event_type == TraceEventType.WORKFLOW_END),
                None
            )
            ...
            if start_event and end_event:
                ...
                self.performance.total_time_ms = (end_time - start_time).total_seconds() * 1000
    ...
    def _update_metrics(self, event: TraceEvent) -> None:
        ...
        if event.agent_name:
            if event.agent_name not in self.performance.agent_metrics:
                self.performance.agent_metrics[event.agent_name] = AgentPerformanceMetrics(
            ...
            if event.event_type == TraceEventType.AGENT_INVOKED:
            ...
            elif event.event_type == TraceEventType.AGENT_COMPLETED and event.duration_ms:
                ...
                self.performance.total_llm_tokens += event.data.get("tokens_input", 0) + event.data.get("tokens_output", 0)
            
            elif event.event_type == TraceEventType.TOOL_INVOKED:
                metrics.tool_calls += 1
            elif event.event_type == TraceEventType.TOOL_COMPLETED and event.duration_ms:
