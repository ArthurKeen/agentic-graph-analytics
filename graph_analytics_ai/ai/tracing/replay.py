...
class TraceReplayer:
    ...
    def _reconstruct_performance(self, perf_data: Dict[str, Any]) -> WorkflowPerformanceMetrics:
    ...
    def replay_timeline(self, speed: float = 1.0, filter_types: Optional[List[TraceEventType]] = None) -> None:
        """
    ...
    def analyze_agent_communication(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'most_active_senders': sorted(agent_sends.items(), key=lambda x: x[1], reverse=True)[:5],
            'most_active_receivers': sorted(agent_receives.items(), key=lambda x: x[1], reverse=True)[:5]
        }
        ...
        if self.trace.performance and other_trace.performance:
            ...
            comparison['performance'] = {
                'total_time_ms': {
                    ...
                    'percent_change': ((this_perf.total_time_ms - other_perf.total_time_ms) / other_perf.total_time_ms * 100) if other_perf.total_time_ms > 0 else 0
                },
                'llm_calls': {
                ...
                'cost': {
                    ...
                    'diff': this_perf.llm_cost_estimate_usd - other_perf.llm_cost_estimate_usd
                }
            }
