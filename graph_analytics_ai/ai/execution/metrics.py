...
class TimingBreakdown:
    ...
    def load_percentage(self) -> float:
        ...
        return (self.graph_load_seconds / self.total_seconds * 100) if self.total_seconds > 0 else 0.0
    
    @property
    def execution_percentage(self) -> float:
        """Percentage of time spent in algorithm execution."""
        return (self.algorithm_execution_seconds / self.total_seconds * 100) if self.total_seconds > 0 else 0.0
    
    @property
    def storage_percentage(self) -> float:
        """Percentage of time spent storing results."""
        return (self.results_store_seconds / self.total_seconds * 100) if self.total_seconds > 0 else 0.0
...
class ExecutionSummary:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "timing_breakdown": self.timing_breakdown.to_dict() if self.timing_breakdown else None,
            "cost_breakdown": self.cost_breakdown.to_dict() if self.cost_breakdown else None,
            "algorithm_stats": {k: v.to_dict() for k, v in self.algorithm_stats.items()},
