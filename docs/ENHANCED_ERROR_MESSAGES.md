...
The library includes comprehensive error messages based on real customer experience, particularly from issues encountered by customer Dima with credential authentication.
...
**Example:**
```python
from graph_analytics_ai import check_password_format
...
```python
from graph_analytics_ai import validate_endpoint_format, check_password_format
...
These enhancements are based on real customer issues encountered by Dima during the psi-graph-analytics project:
